...
#   --help                 Show this help
#
# API mode: REST is preferred and used by default. Falls back to GraphQL
# automatically if the REST endpoint is not available.
#
# Examples:
...
# Tries REST first; falls back to GraphQL. Sets USE_REST=true/false.
USE_REST=false
wait_for_api() {
    local elapsed=0
...
if $MULTI_USER && [[ -n "$TEST_USER_EMAIL" ]]; then
  if $USE_REST; then
    ...
    # REST login returns the JWT string directly (JSON-encoded)
    ...
    # Login to get JWT (no auth header needed — login is unauthenticated)
    log "Logging in as $TEST_USER_EMAIL..."
    ...
    if [[ -n "$USER_JWT" && "$USER_JWT" != "null" ]]; then
        ...
        # Get user's DID for perspective ownership
...
if $DO_SEED && [[ -n "$SEED_FILE" ]]; then
    ...
    if [[ -n "$SEED_NAME_CHECK" ]]; then
        ...
        if $USE_REST; then
            EXISTS=$(rest_get "$CHECK_AUTH" "/perspectives" | python3 -c "
import sys, json
names = [p['name'] for p in json.load(sys.stdin)]
print('$SEED_NAME_CHECK' in names)
" 2>/dev/null) || EXISTS="False"
        else
            EXISTS=$(gql "$CHECK_AUTH" "{\"query\":\"{ perspectives { name } }\"}" | python3 -c "
import sys, json
names = [p['name'] for p in json.load(sys.stdin)['data']['perspectives']]
print('$SEED_NAME_CHECK' in names)
" 2>/dev/null) || EXISTS="False"
        fi
    ...
    if $ALREADY_EXISTS; then
        ...
        if $USE_REST; then
            ...
            log "Importing perspective from $SEED_FILE via REST (as ${TEST_USER_EMAIL:-admin})..."
            SEED_UUID_NEW=$(python3 "$TOOL_DIR/ad4m-perspective-tool.py" \
                --url "${REST_URL}" --auth "$SEED_AUTH" --rest \
                import "$SEED_FILE" $SEED_NAME_ARG) || err "Perspective import failed"
        else
            [[ -f "$TOOL_DIR/ad4m-perspective-tool.py" ]] || err "ad4m-perspective-tool.py not found in $TOOL_DIR"
            log "Importing perspective from $SEED_FILE (as ${TEST_USER_EMAIL:-admin})..."
            SEED_UUID_NEW=$(python3 "$TOOL_DIR/ad4m-perspective-tool.py" \
                --url "$GQL_URL" --auth "$SEED_AUTH" \
                import "$SEED_FILE" $SEED_NAME_ARG) || err "Perspective import failed"
        fi
