...
#
# Examples:
#   ad4m-flux-rebuild.sh --ad4m ./ad4m --flux ./flux
#   ad4m-flux-rebuild.sh --ad4m ./ad4m --flux ./flux --executor --serve 3030
#   ad4m-flux-rebuild.sh --ad4m ./ad4m  # SDK only, no Flux
...
# Auto-detect: script inside repo > workspace defaults
AD4M_DIR=""
if [[ -d "$SCRIPT_DIR/../core" ]]; then
    AD4M_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
elif [[ -d "$DEFAULT_AD4M/core" ]]; then
    AD4M_DIR="$DEFAULT_AD4M"
fi

FLUX_DIR=""
if [[ -d "$DEFAULT_FLUX/app" ]]; then
    FLUX_DIR="$DEFAULT_FLUX"
fi
...
[[ -n "$FLUX_DIR" ]] && FLUX_DIR="$(cd "$FLUX_DIR" && pwd)"  # resolve early

# Step 0: Ensure AD4M monorepo dependencies are installed
if [[ ! -d "$AD4M_DIR/ad4m-ldk/js/node_modules" ]] || ! $SKIP_INSTALL; then
    log "Installing AD4M monorepo dependencies..."
    cd "$AD4M_DIR"
    pnpm install
fi
...
# Step 5: Link Flux → AD4M (temporary — restored after build)
log "Linking Flux → AD4M SDK..."
cd "$FLUX_DIR"

# Save files that get modified during build so we can restore them afterwards.
# This keeps the build script idempotent — no committed files are modified.
cp package.json package.json.bak
[[ -f pnpm-lock.yaml ]] && cp pnpm-lock.yaml pnpm-lock.yaml.bak

restore_flux_files() {
    if [[ -f "$FLUX_DIR/package.json.bak" ]]; then
        mv "$FLUX_DIR/package.json.bak" "$FLUX_DIR/package.json"
    fi
    if [[ -f "$FLUX_DIR/pnpm-lock.yaml.bak" ]]; then
        mv "$FLUX_DIR/pnpm-lock.yaml.bak" "$FLUX_DIR/pnpm-lock.yaml"
    fi
    rm -f "$FLUX_DIR/ad4m"
...
# Step 9: Restore all modified files
# (trap handler covers error/interrupt cases; explicit call for normal exit)
restore_flux_files
trap - EXIT
ok "Restored package.json + pnpm-lock.yaml (overrides removed)"

# Step 10: Serve if requested (NO -s flag — it makes all paths return 200, breaking /health detection)
