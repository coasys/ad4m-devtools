...
#   --graphql                Force GraphQL API mode (dev branch)
#   --rest                   Force REST API mode (REST branch)
#                            Default: auto-detect
#
# Examples:
...
# API mode: "rest" (REST branch) or "graphql" (dev branch). Default: auto-detect.
API_MODE="auto"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --graphql) API_MODE="graphql"; shift;;
        --rest) API_MODE="rest"; shift;;
        -h|--help)
...
# --- Get security code via GraphQL admin API (dev branch) ---
# Uses the agentPermitCapability mutation to permit the request and get the code
get_code_via_graphql() {
    local elapsed=0
    ...
    # Build AuthInfoExtended JSON and call the GraphQL mutation
    local auth_json
    auth_json=$(jq -n \
        --arg rid "$request_id" \
        --arg name "Flux" \
        --arg desc "A Social Toolkit for the New Internet" \
        --arg domain "$FLUX_URL" \
        --arg url "$FLUX_URL" \
        --arg icon "${FLUX_URL}/icon.png" \
        '{
            requestId: $rid,
            auth: {
                appName: $name,
    ...
    log "Calling GraphQL agentPermitCapability..."
    # The mutation takes a JSON string as the auth parameter
    local escaped_auth
    escaped_auth=$(echo "$auth_json" | python3 -c 'import sys,json; print(json.dumps(sys.stdin.read().strip()))')
    local gql_query="mutation { agentPermitCapability(auth: ${escaped_auth}) }"
    local gql_body
    ...
    response=$(curl -sf -X POST "${EXECUTOR_URL}/graphql" \
        ...
        -d "$gql_body" 2>&1) || err "GraphQL permit call failed: $response"

    local code
    code=$(echo "$response" | jq -r '.data.agentPermitCapability // empty' 2>/dev/null) || true

    if [[ ${#code} -eq 6 && "$code" =~ ^[0-9]+$ ]]; then
        ok "Security code from GraphQL: $code"
        echo "$code"
        return 0
    else
        # Might be in the response directly
        warn "Unexpected GraphQL response: $response"
        # Try to extract from executor log as fallback
        code=$(strings "$EXECUTOR_LOG" 2>/dev/null | grep 'Random number challenge' | tail -1 | grep -oE '[0-9]{6}') || true
        if [[ ${#code} -eq 6 ]]; then
            ok "Security code from executor log: $code"
            echo "$code"
            return 0
        fi
        err "Could not get security code from GraphQL or log"
    fi
}
...
# Step 5: Auto-detect API mode if needed
if [[ "$API_MODE" == "auto" ]]; then
    if curl -sf "${EXECUTOR_URL}/api/v1/agent/status" -H "Authorization: ${ADMIN_CREDENTIAL}" >/dev/null 2>&1; then
        API_MODE="rest"
    else
        API_MODE="graphql"
    fi
    log "Auto-detected API mode: $API_MODE"
fi

# Step 6: Get the security code
...
# Step 7: Enter the code and verify atomically via in-page JS
# This calls generateJwt directly from the page context to avoid round-trip delays
log "Entering security code: $CODE"
if [[ "$API_MODE" == "graphql" ]]; then
    # On GraphQL: use the component's own verifyLocalAd4mCode method which
    # correctly stores the JWT with the version-prefixed localStorage key
    cdp_eval "
        (async () => {
            const ac = document.querySelector('ad4m-connect');
            const core = ac?._core || ac?.core;
            ...
            if (!requestId) return 'no requestId';
            try {
                const result = await core.verifyLocalAd4mCode('$CODE');
                if (result) return 'jwt-stored';
                return 'verify-failed';
            } catch(e) {
                // Fallback: try generateJwt directly with proper key storage
                try {
                    const jwt = await core.ad4mClient.agent.generateJwt(requestId, '$CODE');
                    if (jwt) {
                        core.token = jwt;
                        // Use the same version-prefixed key that ad4m-connect expects
                        const ver = (typeof core.version !== 'undefined') ? core.version : '';
                        if (ver) {
                            localStorage.setItem(ver + '/ad4m-token', jwt);
                            localStorage.setItem(ver + '/ad4m-url', core.url);
                        }
                        await core.connect();
                        return 'jwt-stored-fallback';
                    }
                    return 'no-jwt';
                } catch(e2) {
...
while [[ $local_elapsed -lt $TIMEOUT ]]; do
    ...
    if [[ -n "$TOKEN" && "$TOKEN" != "null" && "$TOKEN" != "undefined" && "$TOKEN" != "" ]]; then
        ...
        # Force-connect and hide the overlay instead of reloading
        # Reloading causes ad4m-connect to re-render and block; force-connect
        # from JS keeps the auth state intact
        log "Force-connecting and hiding ad4m-connect overlay..."
        cdp_eval "
            (async () => {
                const ac = document.querySelector('ad4m-connect');
                const core = ac?._core || ac?.core;
                if (core && core.token) {
                    try { await core.connect(); } catch(e) {}
                }
        ...
        " >/dev/null 2>&1
        sleep 5
        # Check if Flux loaded (route changed from /signup)
        FINAL_URL=$(cdp_eval "window.location.href" 2>/dev/null) || true
        if [[ "$FINAL_URL" == *"/home"* || "$FINAL_URL" == *"/community"* ]]; then
            ok "Flux loaded successfully"
        elif [[ "$FINAL_URL" == *"/signup"* ]]; then
            # First-time user — Flux loaded but needs profile creation
            ok "Flux loaded — profile creation screen"
        else
            ok "Flux loaded (url: $FINAL_URL)"
        fi
        exit 0
    fi
    sleep 1
