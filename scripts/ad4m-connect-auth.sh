...
get_code_via_graphql() {
    ...
    if [[ ${#code} -eq 6 && "$code" =~ ^[0-9]+$ ]]; then
        ...
        code=$(strings /tmp/ad4m-executor-stdout.log 2>/dev/null | grep 'Random number challenge' | tail -1 | grep -oE '[0-9]{6}') || true
        if [[ ${#code} -eq 6 ]]; then
...
if [[ "$API_MODE" == "graphql" ]]; then
    # On GraphQL: call generateJwt directly from the page, bypassing the component's
    # verify flow which has timing issues with code expiry
    cdp_eval "
        (async () => {
            const ac = document.querySelector('ad4m-connect');
            const core = ac?._core || ac?.core;
            ...
            try {
                ...
                if (jwt) {
                    core.token = jwt;
                    localStorage.setItem(core.url + '/ad4m-token', jwt);
                    await core.connect();
                    return 'jwt-stored';
                }
                return 'no-jwt';
            } catch(e) {
                // Fallback: try via component
